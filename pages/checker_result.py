...
def render(navigate_to) -> None:
    ...
    for idx, file_pair in enumerate(file_pairs, start=1):
        _render_file_pair_row(index=idx, file_pair=file_pair, navigate_to=navigate_to)
        st.markdown(
...
def _render_table_header(project_a: str, project_b: str) -> None:
    """Render header tabel dengan nama kolom dinamis."""
    ...
    with col_fa:
        label_a = (project_a[:20] + "…") if len(project_a) > 20 else project_a
        st.markdown(f"<div style='{header_style}'>{label_a}</div>", unsafe_allow_html=True)
    with col_fb:
        label_b = (project_b[:20] + "…") if len(project_b) > 20 else project_b
        st.markdown(f"<div style='{header_style}'>{label_b}</div>", unsafe_allow_html=True)
    with col_sim:
...
def _render_file_pair_row(index: int, file_pair, navigate_to) -> None:
    """Render satu baris pasangan file."""
    col_idx, col_fa, col_fb, col_sim, col_view = st.columns([0.5, 3, 3, 2, 1])
    ...
    with col_fa:
        st.markdown(
            f"<div style='padding-top:0.55rem; font-size:0.88rem;"
            f"color:#333; font-family:monospace;'>{file_pair.file_a}</div>",
            unsafe_allow_html=True,
        )
    with col_fb:
        st.markdown(
            f"<div style='padding-top:0.55rem; font-size:0.88rem;"
            f"color:#333; font-family:monospace;'>{file_pair.file_b}</div>",
            unsafe_allow_html=True,
        )
    with col_sim:
